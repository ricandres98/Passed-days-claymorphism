...
let finalDate = document.getElementById('final-date');
    ...
    if (fechaDeNacimiento.year > fechaActual.year ||
        ...
        if(fechaDeNacimiento.year === fechaActual.year){
            if(fechaDeNacimiento.month === fechaActual.month) {
                totalDays = fechaActual.day - fechaDeNacimiento.day;
            }
            else {
                ...
                // (1)
                let mesDeNacimiento = meses.find(function(mes){
                        return mes.numero == fechaDeNacimiento.month;
                });
                console.log(mesDeNacimiento);
                let restantes = mesDeNacimiento.dias - fechaDeNacimiento.day;
        
                // (2)
                let monthsBetween = meses.filter(function(mes){
                ...
                // (3)
                totalDays = restantes + acumDays + fechaActual.day;
            }
        }
        else {
            ...
            let dayNumber = dayOfTheYear(fechaDeNacimiento);
            //Cuenta días para año nuevo del año inicial
            let daysToNY = daysToNewYear(initialYear, dayNumber);
            ...
            let currentDayOfThisY = dayOfTheYear(fechaActual);
            ...
            totalDays = daysToNY + acumDays + currentDayOfThisY;
        }
